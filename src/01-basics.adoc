...
* Ignorieren von nicht ins Repository aufzunehmender Dateien
****
...
1. Passen Sie die Datei *_README.md_* auf GitHub mit folgendem Inhalt an:
+
.README.md
[source,markdown]
----
...
2. Nehmen Sie die Änderung mit kbd:[Commit changes...] als neue Version in das Repository auf.
...
. Öffnen Sie das eben geclonte Projekt (Repository), als Ordner in Visual
...
. Im Bereich *CHANGES* (`git status`) sehen Sie alle _neuen_ *U* (unbeobachtet), _geänderten_
  *M* (modifiziert) und _gelöschten_ *D* (gelöscht) Dateien gegenüber ihrer
  aktuellen Repository-Version. Über das icon:plus[] Symbol (`git add`) bei den
...
Studio Code: "Menü > View > Terminal") den Befehle `asciidoctor` bzw.
`asciidoctor-pdf`.
...
der Fakultät durchgeführt werden. Hier ist die entsprechenden
Asciidoctor-Umgebung installiert.

[IMPORTANT]
======
Studenten des Modules I925 (Studiengang Wirtschaftsingenieurwesen) haben keinen
...
. Passen Sie, falls noch nicht geschehen, ihre Git-Konfig _.gitconfig_ an und
  setzen Ihren Namen und E-Mail. (Siehe
  <<praktikumsaufgaben-teil-01.adoc#section_git_config, Teil
  1 - Git-Konfiguration per Git-Kommando>>):
+
[source,console]
----
$ git config --global user.name "Vorname Nachname"
...
müssen Sie es auf der ilux150 nicht erst clonen, sondern navigieren direkt zu
