...
:image-title: Meanwhile, in the Classroom...
:image-file: comic_basics.jpg
.{image-title}
image::{image-file}["{image-title}", width=700, title="{image-title}"]
...
[#id-multiple-authors]
[NOTE]
.Verwendung von mehreren Autoren
====
...
Der Git-Workflow umfasst das *Abrufen* (`https://git-scm.com/docs/git-pull[git
pull]`), *Bearbeiten*, *Vormerken* (`https://git-scm.com/docs/git-add[git add]`)
und *Festhalten* (`https://git-scm.com/docs/git-commit[git commit]`) von
Änderungen sowie deren *Übertragung* zum Remote-Repository
(`https://git-scm.com/docs/git-push[git push]`). Dazu gehört auch das *Erstellen*
der Arbeitskopie (`https://git-scm.com/docs/git-clone[git clone]`) und das
*Anzeigen* von Änderungen (`https://git-scm.com/docs/git-status[git status]`).

Eine Auflistung von grundlegenden Git-Befehlen finden Sie unter
<<table_basic_git_commands>>.
...
Eine Auflistung von grundlegenden Git-Befehlen mit einer kurzen Beschreibung
und Verlinkung zu den offiziellen Git-Dokumentationen.
...
| `https://git-scm.com/docs/git-clone[git clone] <url>`
| Ein bestehendes Remote-Repository inklusive seiner Dateien und Versionshistorie als lokales Repository herunterladen. Dabei wird standardmäßig ein Verzeichnis mit dem Namen des Repositorys angelegt.

| `https://git-scm.com/docs/git-status[git status]`
| Änderungen im Repository anzeigen

| `https://git-scm.com/docs/git-status[git status] -s`
| Änderungen im Repository in Kurzform (`--short`) anzeigen

| `https://git-scm.com/docs/git-diff[git diff]`
| Änderungen im Repository anzeigen

| `https://git-scm.com/docs/git-add[git add] -A`
| Alle Änderungen (`--all`) im Repository für den nächsten Commit vormerken

| `https://git-scm.com/docs/git-add[git add] -u`
| Änderungen im Repository an bereits von Git erfassten Dateien (`--update`) für den nächsten Commit vormerken

| `https://git-scm.com/docs/git-add[git add] .`
| Änderungen im aktuellen Verzeichnis und darunter für den nächsten Commit vormerken

| `https://git-scm.com/docs/git-add[git add] <file>`
| Angegebene Dateien oder Verzeichnisse für den nächsten Commit vormerken

| `https://git-scm.com/docs/git-restore[git restore] --staged .`
| Alle vorgemerkten Änderungen wieder aus dem nächsten Commit entfernen

| `https://git-scm.com/docs/git-restore[git restore] --staged <file>`
| Vorgemerkte Änderungen an den angegebenen Dateien oder Verzeichnissen wieder aus dem nächsten Commit entfernen

| `https://git-scm.com/docs/git-commit[git commit]`
| Neuen Commit mit Standard-Editor für Commit-Nachricht erstellen

| `https://git-scm.com/docs/git-commit[git commit] -m "..."`
| Neuen Commit mit Commit-Nachricht erstellen

| `https://git-scm.com/docs/git-commit[git commit] -a -m "..."`
| Kurzform für `git add -u` + `git commit -m`

| `https://git-scm.com/docs/git-log[git log]`
| Historie der Commits anzeigen

| `https://git-scm.com/docs/git-log[git log] --oneline`
| Historie der Commits einzeilig anzeigen

| `https://git-scm.com/docs/git-fetch[git fetch]`
| Neue Versionen vom Remote-Repository laden, ohne dies in die lokale Arbeitskopie zu integrieren

| `https://git-scm.com/docs/git-pull[git pull]`
| Neue Versionen vom Remote-Repository laden und in die lokale Arbeitskopie integrieren. Kombination aus `git fetch` + `https://git-scm.com/docs/git-merge[git merge]` / `https://git-scm.com/docs/git-rebase[git rebase]`.

| `https://git-scm.com/docs/git-push[git push]`
| Neue Versionen zum Remote-Repository senden
|===
...
:image-title: Screenshot Generiertes HTML (links) und PDF (rechts)
:image-file: screenshot_html_pdf.png
.{image-title}
image::{image-file}["{image-title}", width=500]
