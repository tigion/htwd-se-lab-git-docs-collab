...
Der Git-Workflow umfasst das Abrufen, Bearbeiten, Vormerken und Committen von
Änderungen sowie deren Übertragung zum Remote-Repository.

.Hinweise: Grundlegende Git-Befehle
[%collapsible]
====
...
| `git clone <url>`             | Ein bestehendes Remote-Repository inklusive seiner Dateien und Versionshistorie als lokales Repository herunterladen. Dabei wird standardmäßig ein Verzeichnis mit dem Namen des Repositorys angelegt.

| `git status`                  | Änderungen im Repository anzeigen
| `git diff`                    | Änderungen im Repository anzeigen

| `git add -A`                  | Alle Änderungen im Repository für den nächsten Commit vormerken
| `git add -u`                  | Änderungen im Repository an bereits von Git erfassten Dateien für den nächsten Commit vormerken
| `git add .`                   | Änderungen im aktuellen Verzeichnis und darunter für den nächsten Commit vormerken
| `git add <file>`              | Angegebene Dateien oder Verzeichnisse für den nächsten Commit vormerken

| `git restore --staged .`      | Alle vorgemerkten Änderungen wieder aus dem nächsten Commit entfernen
| `git restore --staged <file>` | Vorgemerkte Änderungen an den angegebenen Dateien oder Verzeichnissen wieder aus dem nächsten Commit entfernen

| `git commit`                  | Neuen Commit mit Standard-Editor für Commit-Nachricht erstellen
| `git commit -m "..."`         | Neuen Commit mit Commit-Nachricht erstellen
| `git commit -a -m "..."`      | ähnlich wie `git add -u` + `git commit -m`

| `git log`                     | Historie der Commits anzeigen
| `git log --oneline`           | Historie der Commits einzeilig anzeigen

| `git pull`                    | Neue Versionen vom Remote-Repository laden
| `git push`                    | Neue Versionen zum Remote-Repository senden
|===
====
